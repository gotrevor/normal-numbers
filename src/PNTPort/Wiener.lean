import PNTPort.Mathlib.Analysis.Asymptotics.Asymptotics
...
lemma second_fourier_integrable_aux1 (hcont : Measurable ψ) (hsupp : Integrable ψ) (hσ : 1 < σ') :
  ...
  · apply Measurable.aestronglyMeasurable
    ...
    have he : ∀ y : ℝ, ((𝐞 (Multiplicative.ofAdd y) : Circle) : ℂ)
        = Complex.exp (((2 * π * y : ℝ) : ℂ) * Complex.I) := fun _ => rfl
...
-- (vendored dead branch `prelim_decay_2` / `prelim_decay_3` / `decay_alt` removed:
--  they carried `sorry` upstream and nothing in this file depends on them.)
...
theorem sum_le_integral {x₀ : ℝ} {f : ℝ → ℝ} {n : ℕ} (hf : AntitoneOn f (Ioc x₀ (x₀ + n)))
  ...
  have l1 : f (x₀ + 1) ≤ ∫ x in x₀..x₀ + 1, f x := by
    ...
    have h0 : (Ioc x₀ (x₀ + 1))ᶜ ∩ Icc x₀ (x₀ + 1) = {x₀} := by simp [← sdiff_eq_compl_inter]
    rw [mem_ae_iff, Measure.restrict_apply' measurableSet_Icc, h0]
    simp
