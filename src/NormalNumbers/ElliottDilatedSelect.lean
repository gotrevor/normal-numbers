...
/-! ## The dilated finite graph criterion -/

/-- **The dilated graph criterion.**  Dilated analogue of
`NormalNumbers.ElliottTwistedGraph.exists_pairLogCorrelation_small_of_fourier_first_moments`:
small enough Fourier first moments of the *dilated* blocks of `f₁` alone force the affine pair
correlation of `(a n + c₁, a n + c₁ + h)` to be small.

The collision is the dependency's: the lower bound produces `η H/(32 a log H)` and the upper bound,
run at tolerance `η/(2a)`, caps the same quantity by `η H/(64 a log H)`.  The dilation enters only
through the fixed factor `a`, which is chosen before `η`. -/
theorem exists_affineLogCorrelation_small_of_fourier_first_moments
      ...
      ∀ f₁ f₂ : ℕ → ℂ, IsCompletelyMultiplicativeOnPositive f₁ →
        ...
        (∀ j < J, ∀ t : ℤ, logProbExpectation L U (fun n ↦
          ‖blockFourier (a * (4 * h * (a * entropyScale H₀ j) + 1))
            (affineBlock f₁ a n (a * entropyScale H₀ j)) t‖) ≤
            ζ * (a * entropyScale H₀ j : ℕ)) →
        ‖affineLogCorrelation L U f₁ f₂ a (c₁ : ℤ) ((c₁ + h : ℕ) : ℤ)‖ < η := by
  obtain ⟨ζ, hζ, H₁, hH₁, hupper⟩ :=
    exists_dilatedPairTwistedMean_small_of_fourier_first_moment (c₁ := c₁) hh ha
      (show 0 < η / (2 * a) by
...
#print axioms NormalNumbers.ElliottDilatedSelect.exists_affineLogCorrelation_small_of_fourier_first_moments
