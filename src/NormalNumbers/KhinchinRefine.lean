...
/-!
# W5 (Khinchin form) — threading the log-tail zone into the refinement (route C′)

Khinchin analogue of `TBrick.exists_refinement_uniform` (`TBrickRefine.lean:432`):
the refined brick's extension word `u` additionally carries the guarantee
that its empirical log-tail mass past cutoff `K` is `≤ η·n`, extracted from
`x ∉ logBadZone` via the digit-shift identity `logBirkhoffSum_shift`.
-/
...
iterates) into a statement about `cfDigit` values (what the refinement
construction actually tracks). -/
theorem logBirkhoffSum_shift (K n m : ℕ) (x : ℝ) :
    logBirkhoffSum K n (gaussMap^[m] x)
      = ∑ i ∈ Finset.range n,
          (if K < cfDigit x (m + i) then Real.log (cfDigit x (m + i) : ℝ) else 0) := by
  rw [logBirkhoffSum_apply]
  refine Finset.sum_congr rfl fun i _ => ?_
  have hy : gaussMap^[i] (gaussMap^[m] x) = gaussMap^[i + m] x :=
    (Function.iterate_add_apply gaussMap i m x).symm
  rw [hy]
  have hdig : cfDigit x (m + i) = cfDigit (gaussMap^[i + m] x) 0 := by
    have h1 := cfDigit_add x (i + m) 0
    rw [add_zero] at h1
    rwa [show m + i = i + m from add_comm m i]
  unfold logTailFn
  rw [← hdig]

/-- **B–Y Lemma 13, `t' = t` case, Khinchin form.** Mirrors
`TBrick.exists_refinement_uniform` (`TBrickRefine.lean:432`) with the extra
conclusion conjunct: the extension word `u`'s empirical log-tail mass past
cutoff `K` is `≤ η·n` — exactly what `xstar_log_tail_uniform`
(`Khinchin.lean`) needs from the schedule. -/
theorem TBrick.exists_refinement_uniform_khinchin (t : ℕ)
    (F : Finset (List ℕ)) (hF : ∀ v ∈ F, ∀ a ∈ v, 1 ≤ a)
    (hFne : ∀ v ∈ F, v ≠ [])
    {δ ε η : ℝ} (hδ : 0 < δ) (hε0 : 0 < ε) (hεt : (t : ℝ) * ε ≤ 1) (hη : 0 < η)
    {C : ℝ}
    ...
    ∃ kmin₀ : ℕ, ∀ kmin, kmin₀ ≤ kmin → ∃ N K₀ : ℕ,
      ...
      ∃ (B' : TBrick t) (u : List ℕ),
        ...
        (u.map (fun a : ℕ => if K < a then Real.log (a : ℝ) else 0)).sum ≤ η * n := by
  obtain ⟨N₁, kmin₀, K₁, hmain⟩ :=
    exists_good_avoiding_bad_of_large_khinchin t F hF hδ hε0 hεt hη hhalf
  refine ⟨kmin₀, fun kmin hkmin => ?_⟩
  obtain ⟨N₂, hN₂⟩ := exists_fib_threshold (4 * (t : ℝ) ^ kmin)
  refine ⟨max N₁ N₂, K₁, fun B n hn hn0 K hK => ?_⟩
  obtain ⟨x, hxG, hirr, hnotbad⟩ :=
    hmain B n (le_trans (le_max_left _ _) hn) hn0 kmin hkmin K hK
  ...
  · -- the log-tail payload, from `x ∉ logBadZone`
    have hxw : x ∈ cfCylinder B.w := cfCylinder_append_subset _ _ hxJ
    have hnotlog : x ∉ logBadZone B.w n K η :=
      fun h => hnotbad (Or.inr (Or.inr h))
    have horbw : gaussMap^[B.w.length] x ∈ Set.Ioo (0 : ℝ) 1 :=
      (irrational_orbit x hirr hxJ.1 B.w.length).2
    have hle : logBirkhoffSum K n (gaussMap^[B.w.length] x) ≤ η * n := by
      by_contra hlt
      push_neg at hlt
      exact hnotlog ⟨hxw, horbw, hlt⟩
    have hshift := logBirkhoffSum_shift K n B.w.length x
    rw [hshift] at hle
    have hword : (List.range n).map (fun i => cfDigit x (B.w.length + i))
        = u := by
      rw [← hulen]
      exact range_map_cfDigit_eq hxJ
    have hsum_eq : (u.map (fun a : ℕ => if K < a then Real.log (a : ℝ) else 0)).sum
        = ∑ i ∈ Finset.range n,
