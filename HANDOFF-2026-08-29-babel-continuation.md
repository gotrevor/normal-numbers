...
| Mahler minimal sets | `docs/mahler-sets-2026-08-29.md` | `mahler_minimal_sets.py` | 🎯 Single-track (Mahler's setting), base 3 exhaustive m ≤ 12: edge digits need TWO multiples (primitive {1,5}: x or 5x has ternary digit 0 i.o. - π instance free), central digit needs THREE ({1,2,4}); edge meets the B-B lower bound, interior exceeds it; digit-0/2 lists identical (involution cross-check).  🚨 Highest folklore risk of the tower - novelty sweep gates everything here.  Base-4 collapse also landed (6 channels, 676 states, POSITIONED-binary currency: 01/10 re-opened by even-alignment); base-5 run pending |
