...
| Restricted-class ladder | `docs/restricted-class-ladder-2026-08-29.md` | (none yet) | 🪜 Class-restricted quantifiers void adversary lower bounds (B-B / blocking pairs are measure-zero engineered).  💎 Interpolation conjecture: for algebraic irrational x, x or 2x has ternary digit 1 i.o. (between our {1,2,4} theorem and Mahler's open Cantor problem).  New currency: h=0 with POLYNOMIAL-complexity survivors ⟹ theorems for all algebraic irrationals (A-B bridge); the float-zero-exact-FAIL discard pile is the mine.  ⚠️ Serves algebraic instances only - logs stay with the 1/n kick |
| Mahler minimal sets | `docs/mahler-sets-2026-08-29.md` | `mahler_minimal_sets.py` | 🎯 CORRECTED run: EVERY ternary digit needs exactly TWO multiples, {1,2} works for all three (hand proof for digit 1 - the tripwire); 💥 PRODUCT BLOCK {2,11}: **2x or 11x has ALL THREE ternary digits i.o., every irrational x** - first standing joint-visit rung (base-2 two-track version had floored at k=8).  🔬 GATE BUG found+fixed: float h ≤ 1e-3 prefilter before exact check dropped slow-converging true zeros - ALL positives stand (exact-checked), all near-zero NEGATIVES from 1e-3-gated probes are suspect until re-run slack (0.05); base-5 run inherits the caveat.  🚨 Folklore risk PAID OUT: {1,2}-per-digit IS Berend-Boshernitzan 1994's M(3,1)=2, stated explicitly (formalization session's sweep) - C1 reclassified as rediscovery/known-answer test; {2,11} product-block novelty UNKNOWN pending their subsumption check.  Base-4: 6-channel 676-state POSITIONED-binary collapse |
