...
for every m in S and INFINITELY MANY B, then S is not a per-block hitting set (alpha is irrational
because the gaps grow and infinitely many B_j are nonzero).  Berend-Boshernitzan 1994 s3 is the case
B_j = 1 (blocks of m*alpha are the digits of m).

This is a probe, not a proof: it counts B <= BMAX and reports the block with the most avoiders;
growth of the count with BMAX is the evidence for infinitude, and a uniform family (in m) is what a
theorem needs.  Soundness control against the exact automaton (mahler_hitting_set.hits_block): every
