...
| 4 | The weakenings below normality, **which are not a chain**: `normal b → disjunctive b` and `normal b → simplyNormal b`, with neither weakening implying the other, and `disjunctive b → Irrational` while simple normality does **not** force irrationality (`1/3 = 0.010101… ` in base 2 is simply normal and rational) | **partial.**  `IsNormal.isDisjunctive`, `IsDisjunctive`, and its dense-orbit equivalence are formalized; `exists_absolutelyDisjunctive_forall_not_isNormal` gives the generic separation `disjunctive ⇏ normal`.  The other arrows and witnesses remain | **continue.**  The normality implication and generic separation are shipped; the full not-a-chain diagram is still open |
