...
| Axiom M_b ⟹ every quadratic irrational is `b`-disjunctive | **shipped:** `QuadraticHypothesisM` is a named Prop (not a Lean axiom); `missingWordSubshiftDimensionBound` proves the endpoint-safe SFT dimension input and `quadratic_irrationals_disjunctive_of_hypothesisM` proves the exact implication in `QuadraticDisjunctive.lean` | high |
