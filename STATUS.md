...
shrinking ledger.  The two `CFScheduleA` residues become `Prop` nodes on merge.
...
Math-axiom count (🟢+🟡+🟠): **0**.  🔴: none.  The only `src/` `sorry`s are the
two REFUTED `CFScheduleA` schedule residues, which no headline depends on.
